...
def run_wnba():
    ...
    rotoreq = 'https://www.rotowire.com/daily/wnba/api/slate-list.php?siteID=1'

    rotopull = requests.get(rotoreq).json()

    slateid = rotopull['slates'][0]['slateID']
    rotowire = 'https://www.rotowire.com/optimizer/api/wnba/players.php?slateID=' +str(slateid)
    ...
    grind = 'https://rotogrinders.com/api/user-projections?sport=wnba&site=draftkings&user_id=63801&storage=e54bacb7737f0a180ed006df65f75e7b&timestamp=1715796420000&source=3382624'

    pull = requests.get(grind).json()
    rg = pull['data']['source']
    ...
    for key in rg:
        try:
            print(rg[key]['PLAYER'])
            datafra = pd.DataFrame.from_dict([rg[key]])
            frames.append(datafra)
        except:
            pass

    rgdata = pd.concat(frames)                           

    roto_replace ={'Betnijah Laney':'Betnijah Laney-Hamilton','Cheyenne Parker':'Cheyenne Parker-Tyus'}

    rgdata= rgdata.replace(roto_replace)
    ...
    def updateproj(date):
            ...
            rotoinj.loc[rotoinj.player=='Chelsea Gray', 'status'] = 'OUT'
            ...
            def wowyadjustment(team, injuredplayers):
                    ...
                    for i in range(len(injuredplayers)):
                        ...
                        edits.append(pl_id)
                    inj = ",".join(edits)
                    wowy_url2 =f"https://api.pbpstats.com/get-wowy-stats/wnba?0Exactly{total}OffFloor={inj}&Season=2024&SeasonType=Regular%20Season&TeamId={currentteam}&Type=Player"    
            ...
            if minutesprojected == "Y":
                ...
                try:
                    minutes = rgdata[['PLAYER','TEAM','MINUTES']]
                    minutes.columns = ['Name','TeamAbbrev','min']
                    minutes['min'] = minutes['min'].astype(float)
                    minutes = minutes.replace(roto_replace)
                except:
            ...
            reqq = requests.get('https://pick6.draftkings.com/?_ga=2.75507349.1222736360.1716924237-1948912484.1715667395&_gl=1*1js72yi*_ga*MTk0ODkxMjQ4NC4xNzE1NjY3Mzk1*_ga_QG8WHJSQMJ*MTcxNzAxMzY5Mi4zNC4wLjE3MTcwMTM2OTMuNTkuMC4w&sport=WNBA&_data=routes%2F_index',headers =headers)
            
            pick6 = reqq.json()['pickKeyToSinglePlayerPickMap']
            
            pick6lines =[]
            
            for key in pick6:
                test = pick6[key]
                draftables = test['draftableStatLine']
                playerdetail = test['playerDetail']
                playername = playerdetail['displayName']
                line = draftables['targetValue']
                stat = draftables['statCategory']['name']
                props = [playername,stat,line]
                pick6lines.append(props)
            
            dkpick = pd.DataFrame(pick6lines,columns = ['Player','Stat','Line'])
            
            dkpick = pd.pivot_table(dkpick,values = 'Line',index='Player',columns='Stat',aggfunc = sum)
            
            dkpick= dkpick.reset_index()
            ...
            pgcursor.execute( """ DROP TABLE totalgamelogs ;DROP TABLE "PLAYER_GAME_LOGS" ; DROP TABLE "Games";DROP TABLE draftkings
                            ;DROP TABLE playerlast_10; DROP TABLE playerpermin; DROP TABLE playerstdpermin;DROP TABLE prizepicks;
            ...
            try:
                    ...
                    pass
            try:
                    dkpick.to_sql(name ='pick6',schema = 'wnba', con = engine, if_exists = 'replace' , index =False)
            except:
                    pass    
            ...
            try:
                    pgcursor.execute("""select player_name
                        ...
                        end)/10000 ::float as threesover, current_date::varchar as date
                        from (select player_name,min_proj,"TeamAbbrev", ((mai.fgapm * fgapmdef) + ((floor(random() * (100-(-100)+1) -100 ))/100 * fgapmstd)) *  (((floor(random() * (100-(-100)+1) -100 ))/100 * minutesstd) +min_proj ) as fgasim
                        , ((threespm * threespmdef) + ((floor(random() * (100-(-100)+1) -100 ))/100 * threespmstd)) *  (((floor(random() * (100-(-100)+1) -100 ))/100 * minutesstd) +min_proj ) as threessim
                        , ((ftpm * ftpmdef) + ((floor(random() * (100-(-100)+1) -100 ))/100 * ftpmstd)) *  (((floor(random() * (100-(-100)+1) -100 ))/100 * minutesstd) +min_proj )  as ftsim
                        , ((bpm * bpmdef) + ((floor(random() * (100-(-100)+1) -100 ))/100 * bpmstd)) *  (((floor(random() * (100-(-100)+1) -100 ))/100 * minutesstd) +min_proj )  as bpmsim
                    ...
                    ppframe = pd.DataFrame(overs,columns=column)
                    
                    
                    ppframe.to_sql(name = "prizepicksplayerpropmodel",schema = 'wnbastaging',con =engine,if_exists='replace',index=False)
                    
                                
                    pgcursor.execute("""insert into wnba.prizepicksresults 
                    select m.* from wnbastaging.prizepicksplayerpropmodel m 
                    full outer join wnba.prizepicksresults m2 on m.player_name =m2.player_name and  m."date" =m2."date" 
                    where m2.player_name is null;""")
                    
                    
                    pgcursor.execute("""     UPDATE wnba.prizepicksresults mr
                                    SET
                                    "Points" = upd."Points",
                                    pointsover = upd.pointsover,
                                    "Pts+Rebs" = upd."Pts+Rebs",
                                    prover = upd.prover,
                                    "Pts+Asts" = upd."Pts+Asts",
                                    paover = upd.paover,
                                    "Pts+Rebs+Asts" = upd."Pts+Rebs+Asts",
                                    parover = upd.parover,
                                    "Rebounds" = upd."Rebounds",
                                    rebover = upd.rebover,
                                    "Assists" = upd."Assists",
                                    astover = upd.astover,
                                    "Rebs+Asts" = upd."Rebs+Asts",
                                    raover = upd.raover,
                                    "Fantasy Score" = upd."Fantasy Score",
                                    fantover = upd.fantover,
                                    "3-PT Made" = upd."3-PT Made",
                                    threesover = upd.threesover
                                    from (select * from wnbastaging.prizepicksplayerpropmodel) upd
                                    where upd.player_name = mr.player_name  and upd.date = mr.date;""")
            ...
            try:
                    ...
                    udframe = pd.DataFrame(udovers,columns=column)
                    
                    
                    udframe.to_sql(name = "underdogplayerpropmodel",schema = 'wnbastaging',con =engine,if_exists='replace',index=False)
                    
                    
                    pgcursor.execute("""insert into wnba.underdogresults 
                    select m.* from wnbastaging.underdogplayerpropmodel m 
                    full outer join wnba.underdogresults m2 on m.player_name =m2.player_name and  m."date" =m2."date" 
                    where m2.player_name is null;""")
                    
                    pgcursor.execute("""     UPDATE wnba.underdogresults mr
                                    SET
                                    "Points" = upd."Points",
                                    pointsover = upd.pointsover,
                                    "Pts+Rebs" = upd."Pts+Rebs",
                                    ...
                                    "3-PT Made" = upd."3-PT Made",
                                    threesover = upd.threesover
                                    from (select * from wnbastaging.underdogplayerpropmodel) upd
                                    where upd.player_name = mr.player_name  and upd.date = mr.date;""")
            ...
            try:
                lineown =rgdata[['PLAYER','SALARY','POWN']]
                lineown.columns = ['Player','Salary','predicted_ownership']
                
                projupload = pd.merge(rawdata, lineown[['Player','predicted_ownership']],how='left',left_on='Name',right_on='Player')
                projupload['Projected Ownership'] = projupload['predicted_ownership']
                projupload['Projected Ownership'] = projupload['Projected Ownership'].fillna(0).astype(float)
            ...
            projupload.to_csv('wnbaload.csv')
